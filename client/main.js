...
const wordScoreElement = document.getElementById('word-score-board');
const topScoreElement = document.getElementById('top-10-score-board');
...
playButtonElement.addEventListener('click', () => {
  ...
  if (wordIsNotValid(word)) {
    ...
    wordScoreBoard.saveWordScore(getPlayerName(turn), word, scores[turn]);
    wordScoreBoard.render(wordScoreElement);
...
endButtonElement.addEventListener('click', () => {
  for (let i = 1; i <= NUMBER_OF_PLAYERS; i++) {
    gameScoreBoard.saveGameScore(getPlayerName(i - 1), scores[i - 1]);
    scores[i - 1] = 0;
  }
  topWordAndGameScoreBoard.render(topScoreElement);
})
